...
//  const stockDataAuthorable = () => {
//   fetch('https://main--eds-training--sandhiya-narasimhan.aem.page/docs/stock-data.json').
//   then(resp => resp.json()).
//   then(json => {
//       stocks = Object.fromEntries(
//       json.data.map(stock => [stock.Symbol, stock])
//     )
//   })

// }
const childDiv = document.querySelector('.ticker-wrapper div');

childDiv.id = 'ticker-container';
  ...
  Object.keys(stocks).forEach(symbol => {
    ...
    html += `
      <div class="stock-box" data-symbol="${symbol}">
        ...
        <span class="stock-change ${colorClass}">${arrow} ${sign}${parseFloat(data.Change).toFixed(2)}</span>
      </div>
    `;
  });
...
function updateStockPrice(symbol, newPrice) {
  ...
  const oldPrice = parseFloat(stocks[symbol].Price);
  if (oldPrice === newPrice) return; // Do nothing if price hasn't moved

  // Calculate new metrics
  const difference = newPrice - oldPrice;
  stocks[symbol].Price = parseFloat(newPrice);
  stocks[symbol].Change += parseFloat(difference);
  stocks[symbol].isUp = parseFloat(stocks[symbol].Change) >= 0;
  ...
  stockNodes.forEach(node => {
    ...
    changeEl.innerText = `${arrow} ${sign}${parseFloat(stocks[symbol].Change).toFixed(2)}`;
...
// Render the initial UI
 const stockDataAuthorable = (stocksData) => {
  // fetch('https://main--eds-training--sandhiya-narasimhan.aem.page/docs/stock-data.json').
  // then(resp => resp.json()).
  // then(json => {
      stocks = Object.fromEntries(
      ...
      const newPrice = parseFloat(stocks[randomSymbol].Price) + volatility;
